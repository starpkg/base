...
- **Automatic Starlark wiring**: `LoadModule` generates a `set_<name>` builtin for every option and a `get_<name>` builtin for every non-secret option, plus any host-supplied `additionalFuncs`.
...
5. **Backward compatibility (iron rule).** The resolution priority (value > getter > env > default), the generated `set_`/`get_` naming, and the secret/non-secret exposure rules are observable contract for every downstream module and its scripts. Any new lever must default to today's behavior so existing scripts run identically.
...
- **Floor = go 1.19** (`go.mod`), matching the pinned `go.starlark.net` baseline (`ffb3f39`) and `1set/starlet v0.2.1`. The floor only rises in this repo's own isolated pin-upgrade PR.
